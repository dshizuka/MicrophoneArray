...
#I think that the resulting matrix is confusing on two fronts: 
#I belive that the first column is the northing(latitude)
coords.xy=as.data.frame(coords.utm.matrix)
coords.xy[,1]=coords.xy[,1]-min(coords.xy[,1])
coords.xy[,2]=-(coords.xy[,2]-min(coords.xy[,2]))

names(coords.xy)=c("north", "east")
...
plot(loc.result.trim$east, loc.result.trim$north, pch=21, xlim=c(min(loc.result.trim$east-20), max(loc.result.trim$east+20)), ylim=c(min(loc.result.trim$north-20), max(loc.result.trim$north+20)), bg=adjustcolor(sound.type.color[match(loc.result.trim$sound.type, sound.type.color$type), "color"], alpha.f=0.4), las=1, xlab="Easting", ylab="Northing", main=use.folder)
#points(coords.xy, pch="x", col="black", cex=2)
text(coords.xy$east, coords.xy$north,pch="x", col="black", cex=2, labels=rownames(coords.xy))
legend("bottomleft", legend=sound.type.color$type, pch=21, pt.bg=adjustcolor(sound.type.color$color, alpha.f=0.4))


###gganimate
loc.result.trim$sound.type.color=sound.type.color[match(loc.result.trim$sound.type, sound.type.color$type), "color"]
p =ggplot(loc.result.trim, aes(x=east, y=north)) +
  geom_point(aes(fill=sound.type), shape=21, size=5) +
  scale_fill_brewer(palette="Set1") +
  theme_classic() +
  geom_point(aes(x=east, y=north), shape=0, size=5, data=coords.xy) +
  geom_text(aes(x=east, y=north, label=c(1:5)), data=coords.xy) +
  ggtitle('{frame_time}') +
  transition_time(time) +
  shadow_mark(fill="lightgray")
animate(p, fps=1)
anim_save("animation_try.gif")
